...
public class VendingMachine2 {
	...
	private VendingMachineItem2[] itemArray;
	...
	public VendingMachine2() {
		itemArray = new VendingMachineItem2[NUM_SLOTS];
	 ...
	 * @throws VendingMachineException2
	 */
	private int getSlotIndex(String code) throws VendingMachineException2 {
		if( code.equals(A_CODE)) {
			...
			throw new VendingMachineException2(VendingMachine2.INVALID_CODE_MESSAGE);
		}
	}
	 ...
	 * @throws VendingMachineException2 under the following conditions:
	 * 1. If you add an item to a slot that is already occupied. 
	 * 2. If you add an item with an invalid code
	 */
	public void addItem(VendingMachineItem2 item, String code)
			throws VendingMachineException2 {
		int slotIndex = getSlotIndex(code);
		if (itemArray[slotIndex] != null) {
			throw new VendingMachineException2(SLOT_MESSAGE + code
	 ...
	 * @throws VendingMachineException2 if the code is invalid
	 */
	protected VendingMachineItem2 getItem(String code) throws VendingMachineException2 {
		int slotIndex = getSlotIndex(code);
	 ...
	 * @return The item occupying the slot with the given code.
	 * @throws VendingMachineException2 If the slot at the specified code is empty and if the code is invalid
	 */
	public VendingMachineItem2 removeItem(String code) throws VendingMachineException2 {
		int slotIndex = getSlotIndex(code);
		VendingMachineItem2 item = itemArray[slotIndex];
		itemArray[slotIndex] = null;
		if ( item == null) {
			throw new VendingMachineException2(SLOT_MESSAGE + code + IS_EMPTY_MESSAGE);
		}
		return item;
	}
	 ...
	 * @throws VendingMachineException2 Throws a VendingMachineException2 if the amount is < 0 
	 */
	public void insertMoney(double amount) throws VendingMachineException2 {
		if( amount < 0 )
			throw new VendingMachineException2(VendingMachine2.INVALID_AMOUNT_MESSAGE);
		this.balance += amount;
	}
	...
	public boolean makePurchase(String code) {
		...
		VendingMachineItem2 item = getItem(code);
